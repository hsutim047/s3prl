...
class Runner():
        ...
        for dataset in config['sets']:
            ...
            with open(metadata_path, 'r') as f:
                ...
                for i, row in enumerate(rows):
                    if i == 0:
                        continue
                    wavs_name_to_length[row[1]] = int(row[2])
    ...
    def select_hard_wavs(self, all_wavs_list, round_hours, wavs_name_to_length):
        ...
        dataloader = DataLoader(
            ...
            shuffle=False, # eval
            num_workers=self.config['pretrain_expert']['datarc']["num_workers"],
            drop_last=False,
            pin_memory=True,
        ...
        agent_preds = []
        scores = []
        criteria = self.config['runner']['multi_agent']['criteria']
        ...
        while total_wavs_length < round_length:
            ...
            hard_wav = all_wavs_list[idx]
            hard_wavs_list.append(hard_wav)
            total_wavs_length += wavs_name_to_length[hard_wav]
            all_wavs_list.pop(idx)
    ...
    def train(self):
        ...
        # set optimizer
        model_params_agents = [[model] for model in self.upstream.models]
        optimizers = [
